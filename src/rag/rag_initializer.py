...
"""
RAGシステムの初期化を一括で行うモジュール。
以下の処理を順番に実行します：
1. data/sourceディレクトリのファイルをMarkdown化
2. データベースの初期化（PostgreSQLデータベースとテーブルの作成）
3. ベクトルデータベースの更新
4. RAGデータベースの更新
"""
...
def create_vector_database(db_config: Dict[str, Any], markdown_dir: str, reset_table: bool = True) -> VectorDatabase:
    """
...
def create_rag_database(db_config: Dict[str, Any], vector_db_config: Dict[str, Any], reset_table: bool = True) -> RAGDatabase:
    """
...
def initialize_rag_system():
    ...
    # 環境変数のRESET系の設定を全てTrueに設定
    os.environ['RESET_TABLES'] = 'True'
    os.environ['RESET_VECTOR_TABLE'] = 'True'
    os.environ['RESET_RAG_TABLE'] = 'True'
    ...
    # ソースディレクトリとMarkdownディレクトリのパス
    project_root = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))
    source_dir = os.path.join(project_root, 'data', 'source')
    markdown_dir = os.path.join(project_root, 'data', 'markdowns')
    ...
    try:
        # 1. ソースファイルをMarkdown化
        logger.info("ソースファイルをMarkdown化しています...")
        process_documents(
        ...
        logger.info("ソースファイルのMarkdown化が完了しました。")
        
        # 2. データベースの初期化
        logger.info("データベースの初期化を行っています...")
        ...
        # 3. ベクトルデータベースの更新
        vector_db = create_vector_database(vector_db_config, markdown_dir, reset_table=True)
        logger.info("ベクトルデータベースの更新が完了しました。")
        
        # 4. RAGデータベースの更新
        rag_db = create_rag_database(rag_db_config, vector_db_config, reset_table=True)
        ...
        logger.info("RAGシステムの初期化が正常に完了しました。")
...
def main():
    """
    メイン関数。RAGシステムの初期化を実行します。
    """
    try:
        initialize_rag_system()
    except Exception as e:
        logger.error(f"RAGシステム初期化中にエラーが発生しました: {e}", exc_info=True)
